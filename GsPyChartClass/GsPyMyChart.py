...
class GsPyMyChartBase:
    ...
    def __init__(self, fInTableRef: list, fInTableMeas: list, strTitle: str,
        ...
        self._iCountRef: int  # Ilość pomiarów, która jest identyczna w wynikach referencyjnych, jak i rzeczywistych.
...
class GsPyMyChartBox(GsPyMyChartBase):
    ...
    def __createpdf(self):
        ...
        """
        # Rysowanie właściwego wykresu
        fWidthBar: float = self._fX / 6
        for i in range(self._iCountRef):
            self._pdf.set_draw_color(0, 0, 0)
            self._pdf.set_fill_color(0, 0, 255)
            self._pdf.rect(self._myListRef[i][0], self._myListRef[i][1],
                           fWidthBar, self._fHeight - self._myListRef[i][1] + self._fDefOffset + self._fDefTop, "FD")
            self._pdf.set_draw_color(0, 0, 0)
            self._pdf.set_fill_color(255, 0, 0)
            self._pdf.rect(self._myListMeas[i][0] + fWidthBar, self._myListMeas[i][1],
                           fWidthBar, self._fHeight - self._myListMeas[i][1] + self._fDefOffset + self._fDefTop, "FD")
