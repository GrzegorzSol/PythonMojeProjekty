...
class GsPyMyChartBase:
    ...
    def __init__(self, fInTableRef: list, fInTableMeas: list):
        """
        ...
        __fReductionNet: float = 20  # O ile zostanie zmniejszony wykres w pionie i pozimie wzglądem wspólrzędnych
        ...
        self._fSetFontSize: float = 16  # Wymiar czcionki
        ...
        for i in range(self._iCountRef):
            self._myListRef.append((self._fOffset + (self._fX * i), self._fHeight + self._fOffset - (self._fYConvRef * self._fTableRef[i])))
            self._myListMeas.append((self._fOffset + (self._fX * i), self._fHeight + self._fOffset - (self._fYConvRef * self._fTableMeas[i])))
        ...
        # Rysowanie osi X i Y
        self._pdf.set_font("TimesB", size=self._fSetFontSize)
        self._pdf.set_text_color(255, 0, 255)
        ...
        for i in range(self._iCountRef):
            self._pdf.polyline([(self._fOffset, self._fHeight + self._fOffset - (self._fYConvRef * self._fTableRef[i])),
                                (self._fOffset + self._fWidth, self._fHeight + self._fOffset - (self._fYConvRef * self._fTableRef[i]))])
...
class GsPyMyChart(GsPyMyChartBase):
    # Konstruktor
    def __init__(self, fInTableRef: list, fInTableMeas: list, strPathPDF: str):
        """
        @param: fInTableRef: Tablica wyników referencyjnych.
        @param: fInTableMeas: Tablica pomiarów.
        @return: Brak.
        ...
        super().__init__(fInTableRef, fInTableMeas)  # Wywołanie konstruktora klasy przodka
